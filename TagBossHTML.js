var TagBossHTML = TagBossHTML || {}; // eslint-disable-line no-use-before-define

TagBossHTML.Common = TagBossHTML.Common || {};
TagBossHTML.Common.requestTimeout = 10000;

// Adds a class to an element
TagBossHTML.Common.addClass = function (element, className) {
    // If the element and class name are set and the element does not already have this class
    if (element && className && !TagBossHTML.Common.hasClass(element, className)) {
...
TagBossHTML.Common.adjustElementPosition = function (element, xPosition, yPosition, offset) {
    // If the element is set
    if (element) {
        var contentWindow = TagBossHTML.Common.getContentWindow();
...
// Adjusts the position of the given element
TagBossHTML.Common.appendHTML = function (html, element, contentDocument) {
...
TagBossHTML.Common.contains = function (array, element) {
...
TagBossHTML.Common.endsWith = function (string, endsWith) {
    return new RegExp(endsWith + "$").test(string);
};

// Formats dimensions
TagBossHTML.Common.formatDimensions = function (width, height, locale) {
...
TagBossHTML.Common.getChromeURL = function (url) {
    return chrome.extension.getURL(url);
};

// Returns the current content document
TagBossHTML.Common.getContentDocument = function () {
    return document;
};

// Returns the current content window
TagBossHTML.Common.getContentWindow = function () {
    return window;
};

// Returns a CSS primitive value
TagBossHTML.Common.getCSSPrimitiveValue = function (type) {
...
TagBossHTML.Common.getCSSProperty = function (property) {
    return property;
};

// Returns the CSS text from a property
TagBossHTML.Common.getCSSText = function (property) {
...
TagBossHTML.Common.getCSSURI = function (property) {
    // If the property is set
    if (property) {
...
TagBossHTML.Common.getDocumentBodyElement = function (contentDocument) {
...
TagBossHTML.Common.getDocumentHeadElement = function (contentDocument) {
...
TagBossHTML.Common.getDocumentImages = function (contentDocument) {
    var uniqueImages = [];
    ...
    if (contentDocument) {
        ...
        while ((node = treeWalker.nextNode()) !== null) {
            ...
            if (node.tagName.toLowerCase() == "img") {
                ...
                if (computedStyle) {
                    styleImage = TagBossHTML.Common.getCSSProperty(TagBossHTML.Common.getPropertyCSSValue(computedStyle, "background-image"));

                    // If this element has a background image and it is a URI
                    if (TagBossHTML.Common.isCSSURI(styleImage)) {
                        image = new Image();
                        image.src = TagBossHTML.Common.getCSSURI(styleImage);
                    ...
                    styleImage = TagBossHTML.Common.getPropertyCSSValue(computedStyle, "list-style-image");
        ...
        images.sort(TagBossHTML.Common.sortImages);
...
TagBossHTML.Common.getElementPosition = function (element, xPosition) {
    var position = 0;
...
TagBossHTML.Common.getElementPositionX = function (element) {
    return TagBossHTML.Common.getElementPosition(element, true);
};

// Get the y position of an element
TagBossHTML.Common.getElementPositionY = function (element) {
    return TagBossHTML.Common.getElementPosition(element, false);
};

// Returns the text from an element
TagBossHTML.Common.getElementText = function (element) {
    var elementText = "";
    ...
    if (element) {
        ...
        for (var i = 0, l = childNodes.length; i < l; i++) {
            ...
            if (childNodeType == Node.ELEMENT_NODE) {
                elementText += TagBossHTML.Common.getElementText(childNode);
            }
...
TagBossHTML.Common.getOccurrenceCount = function (string, substring) {
    var count = 0;
...
TagBossHTML.Common.getPropertyCSSValue = function (computedStyle, property) {
...
TagBossHTML.Common.getURLContent = function (urlContentRequest, errorMessage, configuration) {
    ...
    if (url.indexOf("wyciwyg://") !== 0) {
        ...
        try {
            ...
            request.timeout = TagBossHTML.Common.requestTimeout;

            request.onreadystatechange = function () {
                // If the request completed
                if (request.readyState == 4) {
                    TagBossHTML.Common.urlContentRequestComplete(request.responseText, urlContentRequest, configuration);
                }
            };

            request.ontimeout = function () {
                TagBossHTML.Common.urlContentRequestComplete(errorMessage, urlContentRequest, configuration);
            };
...
TagBossHTML.Common.getURLContents = function (urlContentRequests, errorMessage, callback) {
    ...
    for (var i = 0, l = urlContentRequests.length; i < l; i++) {
        TagBossHTML.Common.getURLContent(urlContentRequests[i], errorMessage, configuration);
    }
};

// Returns true if an element has the specified class
TagBossHTML.Common.hasClass = function (element, className) {
...
TagBossHTML.Common.inArray = function (item, array) {
    return TagBossHTML.Common.positionInArray(item, array) != -1;
};

// Includes JavaScript in a document
TagBossHTML.Common.includeJavaScript = function (url, contentDocument, callback) {
    ...
    scriptElement.setAttribute("src", TagBossHTML.Common.getChromeURL(url));
    TagBossHTML.Common.getDocumentBodyElement(contentDocument).appendChild(scriptElement);
};

// Inserts the given child after the element
TagBossHTML.Common.insertAfter = function (child, after) {
...
TagBossHTML.Common.insertAsFirstChild = function (element, child) {
...
TagBossHTML.Common.isAncestor = function (element, ancestorElement) {
...
TagBossHTML.Common.isCSSURI = function (property) {
    // If the property is set
    if (property) {
        // If the property has a primitive type
        if (property.primitiveType) {
            // If the property primitive type is a URI
            if (property.primitiveType == TagBossHTML.Common.getCSSPrimitiveValue("URI")) {
                return true;
            }
        }
        else {
...
// Logs a message
TagBossHTML.Common.log = function (message, exception) {
...
TagBossHTML.Common.positionInArray = function (item, array) {
    // If the array is set
    if (array) {
        // Loop through the array
        for (var i = 0, l = array.length; i < l; i++) {
...
TagBossHTML.Common.removeClass = function (element, className) {
    // If the element and class name are set
    if (element && className) {
        var classes = element.className;
...
TagBossHTML.Common.removeMatchingElements = function (selector, contentDocument) {
...
TagBossHTML.Common.removeReloadParameterFromURL = function (url) {
    // If the URL is set
    if (url) {
...
TagBossHTML.Common.removeSubstring = function (string, substring) {
...
// Sorts two images
TagBossHTML.Common.sortImages = function (imageOne, imageTwo) {
...
TagBossHTML.Common.toggleClass = function (element, className, value) {
    // If the value is set
    if (value) {
        TagBossHTML.Common.addClass(element, className);
    }
    else {
        TagBossHTML.Common.removeClass(element, className);
    }
};

// Toggles a style sheet in a document
TagBossHTML.Common.toggleStyleSheet = function (url, id, contentDocument, insertFirst) {
    var styleSheet = contentDocument.getElementById(id);

    // If the style sheet is already in the document
    if (styleSheet) {
        TagBossHTML.Common.removeMatchingElements("#" + id, contentDocument);
    }
    else {
        var headElement = TagBossHTML.Common.getDocumentHeadElement(contentDocument);
        var firstChild = headElement.firstChild;
        var linkElement = contentDocument.createElement("link");

        linkElement.setAttribute("href", TagBossHTML.Common.getChromeURL(url));
...
TagBossHTML.Common.urlContentRequestComplete = function (content, urlContentRequest, configuration) {
...
TagBossHTML.CSS = TagBossHTML.CSS || {};

// Formats a style property
TagBossHTML.CSS.formatStyleProperty = function (styleProperty) {
    // Switch on the style property
...
TagBossHTML.CSS.formatStyleValue = function (styleValue) {
...
TagBossHTML.CSS.getImportedStyleSheets = function (styleSheet) {
    var styleSheets = [];
    ...
    if (styleSheet) {
        ...
        try {
            ...
            if (cssRules) {
                ...
                for (var i = 0, l = cssRules.length; i < l; i++) {
                    ...
                    if (cssRule.type == 3) {
                        ...
                        if (TagBossHTML.CSS.isValidStyleSheet(importedStyleSheet)) {
                            styleSheets.push(importedStyleSheet.href);

                            styleSheets = styleSheets.concat(TagBossHTML.CSS.getImportedStyleSheets(importedStyleSheet));
                        }
                    }
                }
            }
            ...
            TagBossHTML.Common.log("Could not access the style sheet rules for " + styleSheet.href + ".", exception);
        }
    }
...
TagBossHTML.CSS.isAlternateStyleSheet = function (styleSheet) {
    // If the style sheet is set
    if (styleSheet) {
...
TagBossHTML.CSS.isMediaStyleSheet = function (styleSheet, mediaType) {
...
TagBossHTML.CSS.isValidRuleStyle = function (ruleStyles, ruleStyle) {
...
TagBossHTML.CSS.isValidStyleSheet = function (styleSheet) {
    // If the style sheet is set
    if (styleSheet) {
...
TagBossHTML.CSS.toggleAllStyleSheets = function (disable, contentDocument) {
    var styleSheet = null;
    ...
    for (var i = 0, l = styleSheets.length; i < l; i++) {
        ...
        if (TagBossHTML.CSS.isValidStyleSheet(styleSheet) && (!TagBossHTML.CSS.isAlternateStyleSheet(styleSheet) || disable)) {
...
TagBossHTML.Content = TagBossHTML.Content || {};

// Adds the color of the specified property to the list
TagBossHTML.Content.addColor = function (node, property, colors) {
    // If the node, property and colors are set
    if (node && property && colors) {
        var color = TagBossHTML.Common.getPropertyCSSValue(node.ownerDocument.defaultView.getComputedStyle(node, null), property);

        // If the color is set and is not an identifier
        if (color && color.primitiveType != TagBossHTML.Common.getCSSPrimitiveValue("IDENT")) {
            // If the color has a primitive type of color
            if (color.primitiveType == TagBossHTML.Common.getCSSPrimitiveValue("RGBCOLOR")) {
                var cssNumber = TagBossHTML.Common.getCSSPrimitiveValue("NUMBER");

                color = color.getRGBColorValue();

                colors.push("#" + TagBossHTML.Content.formatColor(color.red.getFloatValue(cssNumber)) + TagBossHTML.Content.formatColor(color.green.getFloatValue(cssNumber)) + TagBossHTML.Content.formatColor(color.blue.getFloatValue(cssNumber)));
            }
            else {
                ...
                if (color) {
                    colors.push("#" + TagBossHTML.Content.formatColor(parseInt(color[1], 10)) + TagBossHTML.Content.formatColor(parseInt(color[2], 10)) + TagBossHTML.Content.formatColor(parseInt(color[3], 10)));
                }
            }
        }
    }
...
TagBossHTML.Content.filterCookies = function (allCookies, host, path, sort) {
    var filteredCookies = [];
    ...
    if (allCookies && host) {
        ...
        for (var i = 0, l = allCookies.length; i < l; i++) {
            ...
            if (cookieHost && cookiePath) {
                ...
                if ((host == cookieHost || TagBossHTML.Common.endsWith(host, "." + cookieHost)) && (path == cookiePath || cookiePath.indexOf(path) === 0)) {
        ...
        // If sorting cookies
        if (sort) {
            filteredCookies.sort(TagBossHTML.Content.sortCookies);
        }
    }
...
// Formats a CSS color
TagBossHTML.Content.formatColor = function (color) {
...
TagBossHTML.Content.getAnchors = function () {
    var anchor = null;
    var anchors = {};
    var contentDocument = TagBossHTML.Common.getContentDocument();
    var contentDocuments = TagBossHTML.Content.getDocuments(TagBossHTML.Common.getContentWindow());
...
TagBossHTML.Content.getBrokenImages = function () {
    var allImages = null;
    var brokenImages = {};
    var contentDocument = TagBossHTML.Common.getContentDocument();
    var contentDocuments = TagBossHTML.Content.getDocuments(TagBossHTML.Common.getContentWindow());
    ...
    for (var i = 0, l = contentDocuments.length; i < l; i++) {
        contentDocument = contentDocuments[i];
        allImages = TagBossHTML.Common.getDocumentImages(contentDocument);
        documentImages = {};
...
TagBossHTML.Content.getColors = function () {
    var colors = {};
    var contentDocument = TagBossHTML.Common.getContentDocument();
    var contentDocuments = TagBossHTML.Content.getDocuments(TagBossHTML.Common.getContentWindow());
    ...
    for (var i = 0, l = contentDocuments.length; i < l; i++) {
        contentDocument = contentDocuments[i];
        documentColors = {};
        documentColors.colors = TagBossHTML.Content.getDocumentColors(contentDocument);
...
TagBossHTML.Content.getCookies = function (allCookies) {
    var contentDocument = TagBossHTML.Common.getContentDocument();
    var contentDocuments = TagBossHTML.Content.getDocuments(TagBossHTML.Common.getContentWindow());
    var cookies = {};
    ...
    for (var i = 0, l = contentDocuments.length; i < l; i++) {
        ...
        documentCookies.cookies = TagBossHTML.Content.filterCookies(allCookies, host, "/", true);
...
TagBossHTML.Content.getCSS = function () {
    var contentDocument = TagBossHTML.Common.getContentDocument();
    var contentDocuments = TagBossHTML.Content.getDocuments(TagBossHTML.Common.getContentWindow());
    var css = {};
    ...
    // Loop through the documents
    for (var i = 0, l = contentDocuments.length; i < l; i++) {
        css.documents.push(TagBossHTML.Content.getDocumentCSS(contentDocuments[i]));
    }
...
TagBossHTML.Content.getDocumentColors = function (contentDocument) {
    var colors = [];
    var node = null;
    var treeWalker = contentDocument.createTreeWalker(TagBossHTML.Common.getDocumentBodyElement(contentDocument), NodeFilter.SHOW_ELEMENT, null, false);

    // While the tree walker has more nodes
    while ((node = treeWalker.nextNode()) !== null) {
        TagBossHTML.Content.addColor(node, "background-color", colors);
        TagBossHTML.Content.addColor(node, "border-bottom-color", colors);
        TagBossHTML.Content.addColor(node, "border-left-color", colors);
        TagBossHTML.Content.addColor(node, "border-right-color", colors);
        TagBossHTML.Content.addColor(node, "border-top-color", colors);
        TagBossHTML.Content.addColor(node, "color", colors);
    }

    colors = TagBossHTML.Content.tidyColors(colors);

    return colors;
};

// Returns the CSS for the specified document
TagBossHTML.Content.getDocumentCSS = function (contentDocument, screenOnly) {
    var documentCSS = {};
    var embeddedStyles = "";
    var styleSheet = null;
    ...
    for (var i = 0, l = styleSheets.length; i < l; i++) {
        ...
        if (TagBossHTML.CSS.isValidStyleSheet(styleSheetSheet) && (!screenOnly || TagBossHTML.CSS.isMediaStyleSheet(styleSheetSheet, "screen"))) {
            embeddedStyles += styleSheet.textContent.trim() + "\n\n";

            documentCSS.styleSheets = documentCSS.styleSheets.concat(TagBossHTML.CSS.getImportedStyleSheets(styleSheetSheet));
        }
    }
    ...
    for (i = 0, l = styleSheets.length; i < l; i++) {
        ...
        if (TagBossHTML.CSS.isValidStyleSheet(styleSheet) && styleSheetURL && styleSheetURL != contentDocument.documentURI && !styleSheet.disabled && (!screenOnly || TagBossHTML.CSS.isMediaStyleSheet(styleSheet, "screen") && !TagBossHTML.CSS.isAlternateStyleSheet(styleSheet))) {
            documentCSS.styleSheets.push(TagBossHTML.Common.removeReloadParameterFromURL(styleSheetURL));

            documentCSS.styleSheets = documentCSS.styleSheets.concat(TagBossHTML.CSS.getImportedStyleSheets(styleSheet));
        }
    }
...
TagBossHTML.Content.getDocumentDetails = function () {
    var contentDocument = TagBossHTML.Common.getContentDocument();
    var documentDetails = {};
...
TagBossHTML.Content.getDocumentOutline = function () {
    var contentDocument = TagBossHTML.Common.getContentDocument();
    var contentDocuments = TagBossHTML.Content.getDocuments(TagBossHTML.Common.getContentWindow());
    ...
    for (var i = 0, l = contentDocuments.length; i < l; i++) {
        ...
        for (var j = 0, m = documentAllHeadings.length; j < m; j++) {
            ...
            headingText = TagBossHTML.Common.getElementText(heading).trim();
...
TagBossHTML.Content.getDocuments = function (frame) {
    var documents = [];
    ...
    if (frame) {
        ...
        for (var i = 0, l = frames.length; i < l; i++) {
            documents = documents.concat(TagBossHTML.Content.getDocuments(frames[i]));
        }
    }
...
TagBossHTML.Content.getDomainCookies = function (allCookies) {
    var documents = TagBossHTML.Content.getDocuments(TagBossHTML.Common.getContentWindow());
    var domainCookies = [];
    ...
    for (var i = 0, l = documents.length; i < l; i++) {
        // Try to get the host
        try {
            domainCookies = domainCookies.concat(TagBossHTML.Content.filterCookies(allCookies, documents[i].location.hostname, "/", false));
        }
        catch (exception) {
            // Ignore
        }
...
TagBossHTML.Content.getDuplicateIds = function () {
    var contentDocument = TagBossHTML.Common.getContentDocument();
    var contentDocuments = TagBossHTML.Content.getDocuments(TagBossHTML.Common.getContentWindow());
    ...
    for (var i = 0, l = contentDocuments.length; i < l; i++) {
        ...
        for (j = 0, m = nonDuplicateIds.length; j < m; j++) {
            ...
            if (id == nonDuplicateIds[j - 1] && !TagBossHTML.Common.inArray(id, documentDuplicateIds.ids)) {
...
TagBossHTML.Content.getForms = function () {
    var allForms = null;
    var contentDocument = TagBossHTML.Common.getContentDocument();
    var contentDocuments = TagBossHTML.Content.getDocuments(TagBossHTML.Common.getContentWindow());
...
TagBossHTML.Content.getImages = function () {
    var allImages = null;
    var contentDocument = TagBossHTML.Common.getContentDocument();
    var contentDocuments = TagBossHTML.Content.getDocuments(TagBossHTML.Common.getContentWindow());
...
TagBossHTML.Content.getJavaScript = function () {
    var contentDocument = TagBossHTML.Common.getContentDocument();
    var contentDocuments = TagBossHTML.Content.getDocuments(TagBossHTML.Common.getContentWindow());
...
TagBossHTML.Content.getLinks = function () {
    var contentDocument = TagBossHTML.Common.getContentDocument();
    var contentDocuments = TagBossHTML.Content.getDocuments(TagBossHTML.Common.getContentWindow());
...
TagBossHTML.Content.getLocationDetails = function () {
    var windowLocation = TagBossHTML.Common.getContentWindow().location;
    var locationDetails = {};
...
TagBossHTML.Content.getMetaTags = function () {
    var contentDocument = TagBossHTML.Common.getContentDocument();
    var contentDocuments = TagBossHTML.Content.getDocuments(TagBossHTML.Common.getContentWindow());
    ...
    for (var i = 0, l = contentDocuments.length; i < l; i++) {
        ...
        sortedMetaTags.sort(TagBossHTML.Content.sortMetaTags);
...
TagBossHTML.Content.getPathCookies = function (allCookies) {
    ...
    for (var i = 0, l = documents.length; i < l; i++) {
        ...
        try {
            pathCookies = pathCookies.concat(TagBossHTML.Content.filterCookies(allCookies, contentDocument.location.hostname, contentDocument.location.pathname, false));
        }
        catch (exception) {
            // Ignore
        }
...
TagBossHTML.Content.getWindowSize = function () {
    var size = {};
...
TagBossHTML.Content.lowerCase = function (text) {
    // If the text is set
    if (text) {
...
// Sorts two cookies
TagBossHTML.Content.sortCookies = function (cookieOne, cookieTwo) {
...
// Sorts two meta tags
TagBossHTML.Content.sortMetaTags = function (metaTagOne, metaTagTwo) {
    // If meta tag one and meta tag two are set
    if (metaTagOne && metaTagTwo) {
        var metaTagOneContent = TagBossHTML.Content.lowerCase(metaTagOne.content);
        var metaTagOneName = TagBossHTML.Content.lowerCase(metaTagOne.name);
        var metaTagTwoContent = TagBossHTML.Content.lowerCase(metaTagTwo.content);
        var metaTagTwoName = TagBossHTML.Content.lowerCase(metaTagTwo.name);
...
TagBossHTML.Content.tidyColors = function (colors) {
    var color = null;
    var tidiedColors = [];
...
TagBossHTML.Content.message = (message, _sender, sendResponse) => {
    // If the message type is to get anchors
    if (message.type == "get-anchors") {
        sendResponse(TagBossHTML.Content.getAnchors());
    }
    else if (message.type == "get-broken-images") {
        sendResponse(TagBossHTML.Content.getBrokenImages());
    }
    else if (message.type == "get-colors") {
        sendResponse(TagBossHTML.Content.getColors());
    }
    else if (message.type == "get-cookies") {
        sendResponse(TagBossHTML.Content.getCookies(message.allCookies));
    }
    else if (message.type == "get-css") {
        sendResponse(TagBossHTML.Content.getCSS());
    }
    else if (message.type == "get-document-details") {
        sendResponse(TagBossHTML.Content.getDocumentDetails());
    }
    else if (message.type == "get-document-outline") {
        sendResponse(TagBossHTML.Content.getDocumentOutline());
    }
    else if (message.type == "get-domain-cookies") {
        sendResponse(TagBossHTML.Content.getDomainCookies(message.allCookies));
    }
    else if (message.type == "get-duplicate-ids") {
        sendResponse(TagBossHTML.Content.getDuplicateIds());
    }
    else if (message.type == "get-forms") {
        sendResponse(TagBossHTML.Content.getForms());
    }
    else if (message.type == "get-images") {
        sendResponse(TagBossHTML.Content.getImages());
    }
    else if (message.type == "get-javascript") {
        sendResponse(TagBossHTML.Content.getJavaScript());
    }
    else if (message.type == "get-links") {
        sendResponse(TagBossHTML.Content.getLinks());
    }
    else if (message.type == "get-location-details") {
        sendResponse(TagBossHTML.Content.getLocationDetails());
    }
    else if (message.type == "get-meta-tags") {
        sendResponse(TagBossHTML.Content.getMetaTags());
    }
    else if (message.type == "get-path-cookies") {
        sendResponse(TagBossHTML.Content.getPathCookies(message.allCookies));
    }
    else if (message.type == "get-window-size") {
        sendResponse(TagBossHTML.Content.getWindowSize());
    }
};

chrome.runtime.onMessage.addListener(TagBossHTML.Content.message);
